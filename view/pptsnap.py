...
def convert_ppt_page_to_img(path, page, scale, imgpath=''):
    command = f"libreoffice --headless --convert-to pdf {path} --outdir /tmp/".split()
    ...
    pptname = get_file_name(path)
    pdfpath = f"/tmp/{pptname}.pdf"
    if process.returncode != 0 or not os.path.exists(pdfpath):
        raise Exception(process.returncode)
    else:
        return convert_pdf_page_to_img(pdfpath, page, scale, imgpath)
...
if __name__ == "__main__":
    ...
    scale = args.scale
    print(convert_ppt_page_to_img(path, page, scale, image_path), end="")
